...
keyboard = [
    [
        InlineKeyboardButton(
            JOB_CATEGORIES["COURIER_BUTTON"], callback_data="COURIER_BUTTON"
        )
    ],
    [
        InlineKeyboardButton(
            JOB_CATEGORIES["WAITER_BUTTON"], callback_data="WAITER_BUTTON"
        )
    ],
    [
        InlineKeyboardButton(
            JOB_CATEGORIES["HANDYMAN_BUTTON"], callback_data="HANDYMAN_BUTTON"
        )
    ],
    [
        InlineKeyboardButton(
            SUBMIT_CATEGORY_BUTTON, callback_data=SUBMIT_CATEGORY_BUTTON
        )
    ],
]
...
def start_command(update, context):
    ...
    update.message.reply_text(responses["WELCOME_MESSAGE"], reply_markup=menu_kb)
...
def job_search_confirm_postcode(update, context):
    ...
    found_addresses, postcode_is_correct = gm.find_address_from_input(
    ...
    if found_addresses:
        ...
        for info in data_to_add:
            ...
            logger.info(
                f"{update.message.from_user['id']}: {info} was set to {found_addresses[0][info]}"
            )
        address_summary = responses["ADDRESS_SUMMARY"]
        address_summary = address_summary.replace(
            "{city}", context.user_data["user_data"]["city"]
        )
    ...
    if not postcode_is_correct or not address_summary:
        ...
        update.message.reply_text(
            ...
            reply_markup=ReplyKeyboardMarkup(
                [[YES_BUTTON, NO_BUTTON], [RESTART_BUTTON]], one_time_keyboard=True
            ),
        )
...
def job_search_ask_job_categories(update, context):
    ...
    update.message.reply_text(
        responses["JOB_SEARCH_ASK_CATEGORIES"], reply_markup=job_search_reply_markup
    )
...
def keyboard_callback(update, context):
    ...
    if query.data == SUBMIT_CATEGORY_BUTTON:
        ...
        for category in JOB_CATEGORIES:
            keyboard.append(
                [InlineKeyboardButton(JOB_CATEGORIES[category], callback_data=category)]
            )
        keyboard.append(
            [
                InlineKeyboardButton(
...
def job_search_save_application(update, context, query):
    ...
    context.user_data["user_data"]["timestamp"] = dt.utcnow().replace(
    ...
    for field in ["first_name", "city", "speak_english", "job_category"]:
        if field == "job_category":
            try:
                ...
                job_applicant_summary = job_applicant_summary.replace(
                    "{" + field + "}", selected_jobs_text
                )
            ...
            job_applicant_summary = job_applicant_summary.replace(
                "{" + field + "}", context.user_data["user_data"][field]
            )
        dbservice.update_user_data(
            user_id=context.user_data["user_data"]["user_id"],
            update_dict={field: context.user_data["user_data"][field]},
        )
    ...
    context.bot.send_message(
        chat_id=update.callback_query.message.chat.id, text=job_applicant_summary
    )
    context.bot.send_message(
        chat_id=update.callback_query.message.chat.id,
    ...
    emit_metric(
        type="counters",
        action="unique_unchecked_jobs",
        value=dbservice.count_raw_unchecked_jobs(),
    )
    emit_metric(
        type="counters",
        action="unique_verified_jobs",
        value=dbservice.count_verified_jobs(),
    )
    return WELCOME
...
def parse_job_photo_title(update, context):
    ...
    if not all_raw_jobs:
        update.message.reply_text(responses["PARSE_PHOTO_EMPTY"], reply_markup=menu_kb)
        return WELCOME
    ...
    update.message.reply_text(responses["PARSE_PHOTO_INTRO"])
    ...
    if random_job["description"]:
        update.message.reply_text(random_job["description"])
    else:
        description = ""
        if random_job["file_uri"]:
        ...
        # )
    update.message.reply_text(responses["PARSE_PHOTO_ASK_JOB_TITLE"])
    emit_metric(type="counters", action="unique_users", value=dbservice.count_users())
    emit_metric(
        type="counters",
        action="unique_unchecked_jobs",
...
def parse_job_photo_city(update, context):
    ...
    update.message.reply_text(responses["PARSE_PHOTO_ASK_JOB_CITY"])
...
def parse_job_photo_company(update, context):
    ...
    update.message.reply_text(responses["PARSE_PHOTO_ASK_JOB_COMPANY"])
...
def parse_job_photo_category(update, context):
    ...
    update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            [[COURIER_BUTTON, WAITER_BUTTON, HANDYMAN_BUTTON], [RESTART_BUTTON]],
            one_time_keyboard=True,
        ),
    )
...
def parse_job_photo_thanks(update, context):
    ...
    dbservice.update_raw_job_data(
        context.user_data["current_job_to_parse_id"], update.message.from_user.id
    )

    update.message.reply_text(
        responses["PARSE_PHOTO_THANKS"],
        reply_markup=ReplyKeyboardMarkup(
            [[PARSE_PHOTO_BUTTON], [RESTART_BUTTON]], one_time_keyboard=True
        ),
    )
...
def submit_job_command(update, context):
    update.message.reply_text(responses["SUBMIT_PHOTO_MESSAGE_1"])
    update.message.reply_text(responses["SUBMIT_PHOTO_MESSAGE_2"])
    update.message.reply_text(responses["SUBMIT_PHOTO_MESSAGE_3"])
    update.message.reply_text(
        responses["SUBMIT_PHOTO_MESSAGE_4"],
        reply_markup=ReplyKeyboardMarkup(
            [[UPLOAD_PHOTO_BUTTON, SHARE_LINK_BUTTON]], one_time_keyboard=True
        ),
    )
...
def image_handler(update, context):
    ...
    try:
        ...
        description = (
            "Image caption: {}".format(update.message.caption)
        ...
        if current_job_to_submit_id:
            context.user_data[
                "current_job_to_submit_id"
            ] = current_job_to_submit_id.inserted_id
        update.message.reply_text(
            "By the way, where did you spot this job? Please type in name of the city and full address if possible."
        )
    except Exception as e:
        print(e)
        update.message.reply_text(
            "Something went wrong, please try again later.", reply_markup=menu_kb
        )
        return WELCOME
...
def submit_job_address(update, context):
    ...
    update.message.reply_text(
        "Thank you! We will review this vacancy and if found relevant share with refugees!",
        reply_markup=menu_kb,
    )
    emit_metric(type="etc", action="submit_job_address")
    dbservice.update_raw_job_data_address(
        context.user_data["current_job_to_submit_id"], context.user_data["job_address"]
    )
    emit_metric(type="counters", action="unique_users", value=dbservice.count_users())
    emit_metric(
        type="counters",
...
def format_job(job={}) -> str:
    ...
    if job["updated_at"]:
        res += row_template.format(
            responses["JOB_UPDATED_AT"], job["updated_at"].strftime(datetime_format)
        )
    res += "\n"
    return res
...
def notify_user_about_jobs(context: CallbackContext) -> None:
    ...
    jobs_details = dbservice.get_jobs_by_user_filter(
        user_filter=user_filter, page=page, limit=limit
    )
    ...
    pages = (
        responses["PAGE_TEMPLATE"].format(page + 1, math.ceil(total / limit))
        if total > limit
        else ""
    )
    ...
    context.bot.send_message(
        ...
        text=message,
        reply_markup=ReplyKeyboardMarkup(
            [pagination_buttons, [UNSUBSCRIBE_BUTTON, RESTART_BUTTON]],
            one_time_keyboard=True,
        ),
    )
...
def subscribe_command(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text(
        responses["SUBSCRIBE"], reply_markup=menu_kb_with_unsubscribe
    )
    ...
    context.job_queue.run_repeating(
        name=job_name,
        callback=notify_user_about_jobs,
...
def free_input_command(update, context):
    update.message.reply_text(
        "Sorry, I didn't understand what you said. Try using one of our commands",
        reply_markup=menu_kb,
    )
...
def main():
    ...
    updater = Updater(
        token=os.getenv("API_KEY"), use_context=True, persistence=dict_persistence
    )
    ...
    handler = ConversationHandler(
        ...
        states={
            WELCOME: [
                MessageHandler(
                    filters=Filters.text(JOB_SEARCH_BUTTON),
                    callback=job_search_ask_name,
                ),
                MessageHandler(
                    filters=Filters.text(SUBMIT_JOB_BUTTON), callback=submit_job_command
                ),
                MessageHandler(
                    filters=Filters.text(PARSE_PHOTO_BUTTON),
                    callback=parse_job_photo_title,
                ),
                MessageHandler(
                    filters=Filters.text(SUBSCRIBE_BUTTON), callback=subscribe_command
                ),
                MessageHandler(
                    filters=Filters.text(UNSUBSCRIBE_BUTTON), callback=subscribe_command
                ),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
            ],
            AWAITING_JOB_APPLICANT_NAME: [
                MessageHandler(filters=None, callback=job_search_ask_postcode),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
            ],
            AWAITING_JOB_APPLICANT_POSTCODE: [
                MessageHandler(filters=None, callback=job_search_confirm_postcode),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
            ...
            CONFIRM_JOB_APPLICANT_POSTCODE: [
                MessageHandler(
                    filters=Filters.text(YES_BUTTON), callback=job_search_ask_language
                ),
                MessageHandler(
                    filters=Filters.text(NO_BUTTON), callback=job_search_ask_postcode
                ),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
            ...
            AWAITING_JOB_APPLICANT_LANGUAGE: [
                MessageHandler(filters=None, callback=job_search_ask_job_categories),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
            ...
            SUBMIT_JOB_FLOW: [
                MessageHandler(
                    filters=Filters.text(UPLOAD_PHOTO_BUTTON), callback=submit_photo
                ),
                MessageHandler(
                    filters=Filters.text(SHARE_LINK_BUTTON),
                    callback=submit_job_text_link,
                ),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
            ...
            AWAITING_JOB_PHOTO_TITLE: [
                MessageHandler(filters=None, callback=parse_job_photo_city),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
            ],
            AWAITING_JOB_PHOTO_CITY: [
                MessageHandler(filters=None, callback=parse_job_photo_company),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
            ],
            AWAITING_JOB_PHOTO_COMPANY: [
                MessageHandler(filters=None, callback=parse_job_photo_category),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
            ],
            AWAITING_JOB_PHOTO_CATEGOTY: [
                MessageHandler(filters=None, callback=parse_job_photo_thanks),
                CommandHandler("start", start_command),
                MessageHandler(
                    filters=Filters.text(RESTART_BUTTON), callback=start_command
                ),
    ...
    updater.dispatcher.add_handler(
        MessageHandler(
            filters=Filters.text(JOB_SEARCH_BUTTON), callback=job_search_ask_name
        )
    )
    updater.dispatcher.add_handler(
        MessageHandler(
    ...
    updater.dispatcher.add_handler(
        MessageHandler(
            filters=Filters.text(PARSE_PHOTO_BUTTON), callback=parse_job_photo_title
        )
    )
    updater.dispatcher.add_handler(
        MessageHandler(
